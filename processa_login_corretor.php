...
if (!empty($_POST)) {
    ...
    try {
        // Montar a SQL
        $sql = "SELECT nome, email, telefone, data_nascimento, creci FROM corretores WHERE email =
    :email AND senha = :senha";
        ...
        if ($stmt->rowCount() == 1) { // Se o resultado da consulta SQL trouxer um registro
            ...
            $_SESSION['creci'] = $result['creci'];
            // Redirecionar p/ página inicial (ambiente logado)
            header("Location: index_logado_corretor.php");
            ...
            header("Location: login_corretor.php?msgErro=E-mail e/ou Senha inválido(s).");
        }
    } catch (PDOException $e) {
        die($e->getMessage());
    }
} else {
    header("Location: login_corretor.php?msgErro=Você não tem permissão para acessar esta página..");
}
die();
?>